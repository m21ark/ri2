...
class Basic_Run(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, enable_draw) -> None:
        ...
        self.no_of_actions = act_size = 10
    ...
    def reset(self):
        ...
        for _ in range(25): 
            self.player.scom.unofficial_beam((6+offset,0,0.50),0) # beam player continuously (floating above ground)
            self.player.behavior.execute("Zero_Bent_Knees")
            self.sync()

        # beam player to ground
        self.player.scom.unofficial_beam((6+offset,0,r.beam_height),0) 
        ...
        for _ in range(7):
            self.player.behavior.execute("Zero_Bent_Knees")
            self.player.scom.unofficial_move_ball((-8 + offset/2,offset,0), (0,0,0))
            self.sync()
